		...
		<div className='flex justify-center items-center min-h-screen p-4 bgr-primary'>
			<div className='card border shadow overflow-hidden min-w-[50vw] bg-white'>
				<div className='card-body'>
					<div className='font-bold text-2xl e mb-4 txt-dark'>SIGN UP</div>
					{errorSubmit && (
						<div className='mb-4'>
							<Alert
								severity='error'
					...
					{submitSuccess && <AlertSuccess message='Anda Berhasil Melakukan Registrasi' />}
					<form action=''>
						<div className='grid lg:md:grid-cols-2 gap-4'>
							<FormControl className='w-full'>
								<TextField
									{...register('fullName', { required: '* Nama lengkap harus di isi' })}
									id='outlined-basic'
									label='Nama Lengkap'
									variant='outlined'
								/>
								{errors.fullName && <div className='mt-2 text-end text-red-600 text-xs w-fit'>{errors.fullName.message}</div>}
							</FormControl>
							<FormControl className='w-full'>
								<TextField
							...
							<FormControl
								...
								<OutlinedInput
									...
									label='Password'
								/>
								{!password && <div className='mt-2 text-end text-red-600 text-xs w-fit'>* Password harus di isi</div>}
							</FormControl>
							<FormControl
								className='w-full'
								variant='outlined'>
								...
								/>
								{errorPassword === true && <div className='mt-2 text-end text-red-600 text-xs w-fit'>Konfirmasi Password harus sama</div>}
							</FormControl>
					...
					<div className='text-xs normal-case text-center txt-dark'>
						...
						<span>
							<button
								...
								onClick={() => navigation(`/login/user-account`)}>
								Sign In
							</button>
						</span>
					</div>
